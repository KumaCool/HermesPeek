...
> The `main` installer tracks the current stable Release, v0.2.14. It verifies the fixed wheel against the published `SHA256SUMS` before installing and does not use `sudo`. For a version-fixed installer source, replace `main` with `v0.2.14`.
...
`--plan` is an explicit read-only JSON interface for auditing and automation. Remove it to execute setup; the installer does not invent missing setup values.
